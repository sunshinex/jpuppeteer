...
public class MovementFactory {
    private static final Logger logger = LoggerFactory.getLogger(MovementFactory.class);
    ...
    public MovementFactory(int xDest, int yDest, SpeedManager speedManager,
                           OvershootManager overshootManager, ScreenSize screenSize) {
        this.xDest = xDest;
        this.yDest = yDest;
    ...
    public ArrayDeque<Movement> createMovements(Point currentMousePosition) {
        ...
        if (overshoots == 0) {
            logger.debug("No overshoots for movement from ({}, {}) -> ({}, {})", currentMousePosition.x, currentMousePosition.y, xDest, yDest);
        ...
        while (it.hasNext() && remove) {
            ...
            if (movement.destX == xDest && movement.destY == yDest) {
                ...
                logger.trace("Pruning 0-overshoot movement (Movement to target) from the end. " + movement);
                it.remove();
            } else {
                remove = false;
            }
        ...
        logger.debug("{} movements returned for move ({}, {}) -> ({}, {})", movements.size(), currentMousePosition.x, currentMousePosition.y, xDest, yDest);
        logger.trace("Movements are: {} ", movements);
